...
- **电源开关**：⚠️ 原计划「SPDT 滑动开关物理切断电池」**已被推翻** ——
  ...
  **用户决定：不设独立电池开关，改用三档拨片的「有线」档兼作电源开关** ——
  「有线」档拉高 `SYSOFF` 切断电池：插 USB 时是有线键盘 + 充电，
  **拔掉 USB 即为关机**；另两档电池接通。
  充电 IC 由 `BQ24072` 换成 **`BQ24075`**（封装完全相同，drop-in）。
  见 **ADR-0010**（已决定；`DP3T` 料号与电路细节待定）。
...
- **物理断电**：⚠️ 原「SPDT 串在电池→BAT 之间，额定 ≥2A」的方案**已推翻**：
  ...
  **现方案：换 `BQ24075`（与 `BQ24072` 封装完全相同，pin15 = `SYSOFF`），
  由三档拨片的「有线」档切 `SYSOFF`，不设独立电源开关。**
  并用一颗 `2N7002` 保证插 USB 时仍能充电（`SYSOFF` 拉高会同时禁用充电）。
  ⚠️ 记账：这是**低漏电**（约 4.3 µA）而**非电气隔离**。见 **ADR-0010**（已决定）。
