...
- 容量 **10 Ah** 下，芯片可编程上限 **1.5 A ÷ 10 Ah = 0.15C**，远在 LiPo 理想区间
  ...
  卡点在 USB 声明、散热与**安全定时器**。
- ⚠️ **建议编程 0.5–1.0 A**：BQ24072 是线性充电器，1.5 A 时耗散约
  (5 − 3.7) × 1.5 ≈ **2 W**，会触发 thermal regulation 自动降流并发热。
- `K_ISET = 890`（TI SLUA947A）→ R_ISET：1.5 A = 593 Ω / 1.0 A = 890 Ω / 0.5 A = 1.78 kΩ。
- 🔴 **安全定时器是新的硬约束**（2026-09-17 容量变更后才发现）：
  `tMAXCHG` 最长 = `10 × R_TMR(72 kΩ 上限) × K_TMR(60 s/kΩ 上限)` = **12 h**（typ 9.6 h），
