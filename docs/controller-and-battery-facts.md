...
- **1.5 A ÷ 3.0 Ah = 0.5C**，落在 LiPo 理想区间。厂商标称「标准充电 0.5C–1C（1.5–3 A）」，
  即 1.5 A 是其标称区间**下边界**。✅ **不超限。**
- ⚠️ 但**建议编程 0.5–1.0 A**：BQ24072 是线性充电器，1.5 A 时耗散约
  (5 − 3.7) × 1.5 ≈ **2 W**，会触发 thermal regulation 自动降流并发热。
- `K_ISET = 890`（TI SLUA947A）→ R_ISET：1.5 A = 593 Ω / 1.0 A = 890 Ω / 0.5 A = 1.78 kΩ。
...
| C2 | 决策矩阵引脚方案（74HC595 等）并写 ADR | 需用户决策 |
