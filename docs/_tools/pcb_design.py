...
CONTROL = {
    ...
    # 三档模式开关（ADR-0009）—— 2026-09-19 用户定：右上角；同日再按「使用体验」微调
    # ⚠️ 位置演进：(75,12) → (360,6)「右上角」→ **(350,11)**：用户反馈「太偏了」，
    #    往左下各挪 10mm / 5mm，别顶着角放。
    # 封装姿态（读 KiCad 官方 SW_SP3T_PCM13 实测，不是猜的）：
    #    本体 9.7 × 2.55 mm（沿 X 长）；**四个信号脚在 −Y 侧**（y = −1.43）；
    #    **操作柄在 +Y 侧伸出**（F.CrtYd 到 y = 3.4，本体面在 1.6）⇒ rot=0 时柄朝
    #    「+Y = 指向键区方向」。若日后希望柄朝键盘后壁（−Y），把 rot 改成 180 即可。
    # ⚠️ 代价：开关到 nice!nano 的 MODE0/1/2 三条线（J3A.5 / J3B.10 / J3B.8）约 310 mm。
    #    开关本身只走 µA 级信号，电气上无碍；三条线在顶肩里走，空间充裕。
    "SW96": (350.00, 11.00, 0.0),
