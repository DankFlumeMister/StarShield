...
**状态：🟢 已决定（用户 2026-09）—— 门控拓扑定为「拓扑 C」**
**门控实现（已定）**：`2N7002`（NMOS）反相级 + `AO3401A`（P-FET）在**电池轨**，
ZMK 侧 `GPIO_ACTIVE_HIGH`；**照抄 `kurtis-lew/Conejo` 的接法**
（`S=VDDH` 电源、`D=EXT_PWR` 负载，**体二极管方向正确**）。
⚠️ **不要照抄 ZMK 硬件设计指南的 Q3** —— 它把 `S`/`D` 画反了（详见下方勘误段）。

用户要求每键独立发光，并能像商业键盘一样用组合键控制颜色/灯效/亮度。

RGB 是无线键盘的最大功耗源：95 颗可寻址 LED 即使不发光，芯片待机也可能持续消耗约 95mA，是主控深度睡眠电流（约 20µA）的数千倍。若不处理，电池会被待机漏电迅速耗尽（本项目已改为 10000 mAh 容量，按此漏电也只够约 4 天）。
