...
**状态：🟢 已决定（用户）—— 机制与开关形态已定（单刀 SP3T）**
**日期：2026-09**
**取代：ADR-0001 中「Dongle 作为 ZMK 分体 central」的隐含假设（见「对 ADR-0001 的影响」）**
**关联：ADR-0010 —— 已决定**不设电池断电**，故本开关只需单刀，不涉及 `SYSOFF`**
...
ADR-0010 决定**不设电池断电**，因此本开关**只需单刀**，不用双刀 `DP3T`。
可用料号：
...
> 🔴 **绝对不要用 E-Switch `EG1312AR`**：厂家页面明确写着第三档是
> **瞬时（MOMENTARY）**动作（`ON-ON-(ON)`，*"右侧位置具有瞬时功能"*）。
> 它**不能**当作保持型三档模式选择器 —— 这是个会让人白画一版 PCB 的坑。

> 📌 **尺寸对比**：`PCM13`（9.7 × 2.55 mm）比常见 `MSK12C02`（本体 **8 × 2.8 × 1.4 mm**；
> 注意 1.4 mm 是本体高，另 1.5 mm 是手柄高）**略长但更薄**，属可接受量级。
> 选 `PCM13` 的主要理由是**官方 KiCad 封装现成**。

## 风险与未决项

- 🔴 **开关料号尚未最终定案**：本 ADR 的机制只需**单刀 `SP3T`**。
  ADR-0010 已决定**不设电池断电**，所以**不需要**双刀 `DP3T`
  —— 这消掉了原先「单刀 vs 双刀必须一起选型」的耦合。
  现在的选择只在 `C&K PCM13`（有官方封装）与 Alps `SSSS213800`（有出货实证）之间。
