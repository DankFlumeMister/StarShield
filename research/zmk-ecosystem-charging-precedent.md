...
> ## ⚠️ 本报告的 §5「建议维持 0.5 A」存在样本偏差，已于 2026-09-18 修正
>
> 本报告的样本（nRFMicro / mikoto / puchi_ble）**全是 90–500 mAh 的小电池设备**
> （ZMK 生态的无线板几乎都是分体键盘）。它们的 350 mA 相当于 **0.7–1C**，
> **不能用来说明「10000 mAh 该用多大电流」** —— 那是把两个数量级混为一谈。
>
> ⇒ **同容量段（≥5000 mAh）的证据见 `large-battery-keyboard-charging-current.md`**：
> 商用产品实际用 **0.035–0.163C（350–1400 mA）**，本项目 0.049C 落在**下三分之一**。
> P13 的最终判断以那份报告为准。
>
> ✅ **本报告其余结论仍然成立，并已被后续证据加强**：
> - 「消费级充电 IC 无充电安全定时器」→ 商用拆解实证 `TP4056` 同样没有（C/10 终止）
> - 「不用电池温度监测是常态」→ 商用拆解实证：**TP4056 有 TEMP 脚，厂商仍用 2 线电池包**
